...
class WindowsPrinterManager(PrinterManager):
    ...
    def _find_available_driver(self):
        ...
        try:
            ...
            ], universal_newlines=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
    ...
    def _create_port(self, port_name, ip, port):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=30, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
    ...
    def _restart_spooler(self):
        ...
        try:
            subprocess.run(['net', 'stop', 'spooler'], capture_output=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            time.sleep(1)
            subprocess.run(['net', 'start', 'spooler'], capture_output=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            time.sleep(2)
    ...
    def _set_printer_comment(self, name, comment):
        ...
        try:
            ...
            subprocess.run(cmd, capture_output=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
        except:
            pass
    ...
    def remove_printer(self, name):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            if result.returncode == 0:
                success = True
...
class UnixPrinterManager(PrinterManager):
    ...
    def _check_cups_availability(self):
        ...
        try:
            result = subprocess.run(['which', 'lpadmin'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            if result.returncode != 0:
                return False
            
            result = subprocess.run(['which', 'lpstat'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            if result.returncode != 0:
                return False
            
            try:
                result = subprocess.run(['lpstat', '-r'], capture_output=True, text=True, timeout=5, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
    ...
    def add_printer(self, name, ip, port, printer_port_name=None, make_default=False, comment=None):
        ...
        for driver in drivers:
            try:
                ...
                result = subprocess.run(current_cmd, capture_output=True, text=True, timeout=30, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
        ...
        try:
            if comment:
                ...
                subprocess.run(cmd_comment, capture_output=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
        except Exception as e:
    ...
    def remove_printer(self, name):
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
            if result.returncode == 0:
    ...
    def check_printer_exists(self, name):
        ...
        try:
            result = subprocess.run(['lpstat', '-p', name], capture_output=True, timeout=5, creationflags=subprocess.CREATE_NO_WINDOW if self.system == "Windows" else 0)
