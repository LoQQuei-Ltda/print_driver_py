...
class DocumentListPanel(wx.Panel):
    ...
    def _init_ui(self):
        ...
        self.document_list.InsertColumn(3, "Ações", width=200)
    ...
    def load_documents(self):
        """Carrega os documentos disponíveis para impressão"""
        try:
            self.document_list.DeleteAllItems()
            self.document_list.InsertItem(0, "Carregando...")
            wx.GetApp().Yield()

            self.documents = [] # TODO: fazer lógica para coletar os documentos

            if self.documents and len(self.documents) > 0:
                for i, doc in enumerate(self.documents):
                    date_str = self._format_date(doc.get("created_at", ""))

                    size_str = self._format_size(doc.get("size", 0))

                    index = self.document_list.InsertStringItem(i, doc.get("name", "Sem nome"))
                    self.document_list.SetItem(index, 1, date_str)
                    self.document_list.SetItem(index, 2, size_str)

                    self._create_action_buttons(doc.get("id"), index)
    ...
    def _create_action_buttons(self, document_id, index):
        ...
        def on_print(event):
            ...
            # TODO: fazer lógica para pegar as impressoras
            printers = [
                {"id": "printer1", "name": "Impressora 1"},
            ]

            choices = [p["name"] for p in printers]

            dialog = wx.SingleChoiceDialog(
                self,
                "Selecione a impressora para imprimir o documento:",
                "Imprimir documento",
                choices
            )
            ...
            if dialog.ShowModal() == wx.ID_OK:
                ...
                printer_id = printers[selected_index]["id"]

                self.on_print(document_id, printer_id)
